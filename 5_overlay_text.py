...
bg_audio_path = os.path.join(mix_audio_dir, 'bg.mp3')
...
fps = video_clip.fps
intro_duration = 5
intro_text = TextClip("Hello World", fontsize = 70, color='white',  size=video_clip.size, font = 'Courier' )
...
watermark_size = 60
watermark_text = TextClip("CFE", fontsize=watermark_size, color='white', align='East', size=(w,watermark_size))
watermark_text = watermark_text.set_fps(fps)
watermark_text = watermark_text.set_duration(video_clip.reader.duration)
watermark_text = watermark_text.margin(left=10, bottom=10, opacity=2)
